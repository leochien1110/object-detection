...
# ref: https://github.com/dschwalm/deeplearning/blob/master/opencv/yolov4_webcam.py
...
def get_yolo_preds(net, video_url, confidence_threshold, overlapping_threshold, labels = None, frame_resize_width=None):
    ...
    Args:
        net (cv2.dnn.readNet): Neural Network
        video_url: Video input
        confidence_threshold (int): Only show the bbox with confidence higher than this threshold
        overlapping_threshold (int): Prevent from showing mutliple bboxes for the same object
        labels (.names file, optional): Object lable list. Defaults to None.
        frame_resize_width (int, optional): video width. Defaults to None.
    """
